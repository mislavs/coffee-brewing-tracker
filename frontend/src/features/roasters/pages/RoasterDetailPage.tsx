...
import { useDeleteRoaster } from '@/features/roasters/hooks/useDeleteRoaster'
...
function RoasterDetailContent({ roasterId }: { roasterId: Guid }) {
  ...
  const { mutateAsync: deleteRoaster, isPending: isDeleting } = useDeleteRoaster()
  ...
  const confirmDelete = async () => {
    try {
      await deleteRoaster(roasterId)
      setIsDeleteDialogOpen(false)
      navigate('/roasters')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Could not delete roaster.')
    }
  }
