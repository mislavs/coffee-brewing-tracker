...
async function resolveViteApiUrl() {
  ...
  return readEnvValueFromFile(path.resolve(cwd, '.env'), 'VITE_API_URL')
}

const cliUrl = getCliArg('--url')
const openApiUrlFromEnv = process.env.COFFEE_TRACKER_OPENAPI_URL
const viteApiUrl = await resolveViteApiUrl()

const candidates = [
  cliUrl,
  openApiUrlFromEnv,
  viteApiUrl ? toOpenApiUrl(viteApiUrl) : undefined,
...
if (!payload || !resolvedUrl) {
  throw new Error(
    [
      'Could not download OpenAPI spec from any configured endpoint.',
      ...failures,
    ].join('\n'),
  )
}
